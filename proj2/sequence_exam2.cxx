...
bool correct(sequence &test, size_t size, size_t cursor_spot, double items[])
{
    ...
    {
        cout << "Basic test of size() or is_item() failed." << endl
             << endl;
        return false;
    }
    ...
    {
        cout << "Test of the sequence's items failed." << endl
             << endl;
        return false;
    }
    ...
    cout << "All tests passed for this sequence." << endl
         << endl;
    return true;
}
...
int test5()
{
    ...
    if (!correct(copy2, 2 * original.DEFAULT_CAPACITY, 2 * original.DEFAULT_CAPACITY - 1, items))
        return 0;
    ...
    original.advance();
    original.remove_current(); // Removes 2 from the original, but not the copy.
    if (!correct(copy5, 2 * original.DEFAULT_CAPACITY, 2 * original.DEFAULT_CAPACITY, items))
        return 0;
...
int test7()
{
    ...
    for (i = 1; i <= 2 * testa.DEFAULT_CAPACITY; i++)
        testa.attach(i);
    if (!correct(testa, 2 * testa.DEFAULT_CAPACITY, 2 * testa.DEFAULT_CAPACITY - 1, items))
        return 0;
